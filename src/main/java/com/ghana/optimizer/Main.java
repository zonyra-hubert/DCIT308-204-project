...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("==========================================================================");
        System.out.println("System Parameteresult Initialized:");
        ...
        try (Connection connection = ConnectionManager.getConnection();
            ...
            ResultSet result = stmt.executeQuery("SELECT COUNT(*) FROM locations;");
            if (result.next()) {
                System.out.println("  -> Campus Nodes/Locations Loaded: " + result.getInt(1));
            }

            result = stmt.executeQuery("SELECT COUNT(*) FROM roads;");
            if (result.next()) {
                System.out.println("  -> Campus Road Segments Loaded: " + result.getInt(1));
            }

            result = stmt.executeQuery("SELECT COUNT(*) FROM service_requests;");
            if (result.next()) {
                System.out.println("  -> Active Service Requests Loaded: " + result.getInt(1));
            }

            result = stmt.executeQuery("SELECT COUNT(*) FROM resources;");
            if (result.next()) {
                System.out.println("  -> Campus Maintenance/IT Resources Loaded: " + result.getInt(1));
            }
